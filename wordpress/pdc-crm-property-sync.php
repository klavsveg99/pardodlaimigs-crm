...
/*
 * ============================================================
 * Properties → CRM JSON Feed (wp-json/crm/v1/properties)
 *
 * Exposes ALL WP property posts to the CRM (pdc:backfill /
 * ReconcileAllProperties job, crm.properties_cache). The CRM's
 * PropertyNormalizer expects the mu-plugin item shape:
 * ID/post_title/post_status + meta ere_* + terms property-*.
 * The real_estate_property_* meta keys are mapped onto those.
 * Authenticated with the same X-CRM-API-Key.
 * ============================================================
 */
add_action('rest_api_init', function () {
    register_rest_route('crm/v1', '/properties', [
        'methods' => 'GET',
        'callback' => 'crm_properties_feed',
        'permission_callback' => 'crm_wpforms_authenticate',
        'args' => [
            'page' => ['default' => 1, 'sanitize_callback' => 'absint'],
            'per_page' => ['default' => 100, 'sanitize_callback' => 'absint'],
...
/**
 * Maps real_estate_property_* metas + property-* taxonomies onto the
 * ere_* meta shape the CRM normalizer reads.
 */
function crm_property_feed_row(WP_Post $post): array
{
    $id = (int) $post->ID;

    $raw = static function (string $key) use ($id): string {
    ...
    $price = preg_replace('/[^\d.,]/', '', $raw('real_estate_property_price'));
    if ($price !== '' && (float) str_replace(',', '.', $price) > 0) {
        $meta['ere_property_price'] = $price;
    }
    $meta['ere_property_price_prefix'] = 'EUR';

    foreach ([
        'ere_property_bedrooms' => 'real_estate_property_bedrooms',
        'ere_property_bathrooms' => 'real_estate_property_bathrooms',
        'ere_property_area' => 'real_estate_property_size',
        'ere_property_land_area' => 'real_estate_property_land',
        'ere_property_address' => 'real_estate_property_address',
        'ere_property_country' => 'real_estate_property_country',
    ] as $target => $source) {
        $value = $raw($source);
        if ($value !== '') {
            $meta[$target] = $value;
    ...
    // Real-estate land values are stored in hectares — the CRM normaliser
    // applies the unit key to convert to m².
    $meta['ere_property_land_area_unit'] = 'ha';

    // Location meta holds "lat,lng" (street address is in the address meta).
    $location_meta = get_post_meta($id, 'real_estate_property_location', true);
    $latlng = '';
    if (is_array($location_meta)) {
        $latlng = (string) ($location_meta['location'] ?? '');
    } else {
        $latlng = (string) $location_meta;
    }
    $parts = array_map('trim', explode(',', (string) $latlng));
    if (count($parts) === 2 && is_numeric($parts[0]) && is_numeric($parts[1])) {
        $meta['ere_property_latitude'] = $parts[0];
        $meta['ere_property_longitude'] = $parts[1];
    }

    // Gallery: CRM reads _thumbnail_id (numeric) + ere_property_gallery
    // (comma-separated IDs) + an ere_property_gallery_<id> URL per image.
    $gallery_ids = array_values(array_filter(
        array_map('trim', explode('|', $raw('real_estate_property_images'))),
        'ctype_digit'
    ));

    if ((int) $raw('_thumbnail_id') > 0) {
        $meta['_thumbnail_id'] = (string) (int) $raw('_thumbnail_id');
    }
    if ($gallery_ids !== []) {
        $meta['ere_property_gallery'] = implode(',', $gallery_ids);
        foreach ($gallery_ids as $gid) {
            ...
            if ($url) {
                $meta['ere_property_gallery_'.$gid] = $url;
            }
        }
    }

    $meta = array_merge($meta, crm_property_feed_terms_meta($id));
    ...
    return [
        ...
        'meta' => $meta,
        'terms' => crm_property_feed_terms($id),
    ];
}

/** Term name lookup used to fill city/neighborhood meta keys. */
function crm_property_feed_terms_meta(int $id): array
{
    $fill = [
        'ere_property_city' => 'property-city',
        'ere_property_neighborhood' => 'property-neighborhood',
        'ere_property_state' => 'property-state',
    ];

    $meta = [];
    foreach ($fill as $target => $tax) {
        $terms = get_the_terms($id, $tax);
        if (is_array($terms) && $terms !== []) {
            $meta[$target] = $terms[0]->name;
        }
    }
 ...
 */
function crm_property_feed_terms(int $id): array
{
    $terms = [];

    foreach (['property-status', 'property-type', 'property-feature', 'property-label'] as $tax) {
        $rows = [];
        $terms_list = get_the_terms($id, $tax);
...
function crm_properties_feed(WP_REST_Request $request)
{
    $page = max(1, (int) $request->get_param('page'));
    $per_page = min(100, max(1, (int) $request->get_param('per_page')));
    ...
    foreach ($query->posts as $post) {
        $rows[] = crm_property_feed_row($post);
    }
